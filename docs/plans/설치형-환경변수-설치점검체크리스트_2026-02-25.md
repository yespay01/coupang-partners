...
## 3.5. Root Compose ENV 설정
파일: `.env` (루트, docker compose용)
참고 예시: `.env.example`

주요 항목:
- `STACK_NAME` (컨테이너/볼륨 prefix)
- `POSTGRES_PASSWORD`, `MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD`
- `*_HOST_PORT` (운영/테스트 동시 실행 시 포트 분리)
...
4. 루트 `.env` 작성 (`STACK_NAME`, `*_HOST_PORT`, 비밀번호 등)
5. 필요 시 `docker-compose.override.yml` 작성 (예시: `docker-compose.override.example.yml`)
6. 도메인/DNS 연결
7. 리버스 프록시 + SSL 설정
8. 컨테이너 실행
9. 관리자 로그인 확인 (`/admin`)
10. 설정 > 사이트 정보 입력
11. 쿠팡 API / AI / 자동화 설정 입력
12. 수동 수집/리뷰 생성 테스트
13. 크론 로그 확인
...
## 10. Compose 멀티파일 주의사항
- `ports`는 override 파일에서 재정의 시 배열 병합(append)으로 동작할 수 있어 충돌 원인이 됩니다.
- 포트 분리는 override보다 루트 `.env`의 `*_HOST_PORT`를 사용하세요.
