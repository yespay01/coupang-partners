...
> 최종 업데이트: 2026-02-10
> 작업 분리: 개발 컴퓨터(코드 작업) / 서버 컴퓨터(배포 전용)
> Git 기반 워크플로우
...
### 1. 코드 수정 절대 금지
- 서버에서 직접 코드를 수정하지 않습니다
- 모든 코드 변경은 개발 컴퓨터에서 진행
- **Git pull로만** 코드를 받아옵니다

### 2. Git Pull + 배포만 담당
- 개발쪽에서 Git push 완료 통보 시:
  1. `git pull` (코드 받기)
  2. `docker-compose build` (이미지 빌드)
  3. `docker-compose up -d` (서비스 실행)

### 3. 대기 상태 유지
- 개발쪽에서 요청이 올 때까지 대기
- 자동으로 pull 받거나 배포하지 않음
...
개발쪽에서 "Git push 완료, 배포 진행" 통보를 받으면:
...
### 4. 결과 보고
- 성공: "✅ 배포 완료 - Git pull 성공, 컨테이너 정상 실행 중"
- 실패: "❌ 배포 실패 - [에러 메시지]"
...
### 1. "Git push 완료, 배포 진행"
→ `git pull` → 위 표준 배포 절차 실행

### 2. "환경변수 변경: [항목=값]"
→ .env.production 수정 후 재배포

### 3. "로그 확인 요청"
→ `docker-compose logs` 결과 전달
...
- [ ] 개발쪽에서 "Git push 완료" 통보 받음
- [ ] `/home/insuk/blog/` 디렉토리에서 `git pull` 실행
- [ ] Git pull 성공 확인 (충돌 없음)
- [ ] .env.production 파일 존재 및 내용 확인

배포 후 확인사항:

- [ ] 컨테이너 실행 중 (`docker ps`)
- [ ] 로그 에러 없음 (`docker-compose logs`)
...
| 날짜 | 내용 |
|------|------|
| 2026-02-03 | 초기 지침 작성 (파일 전송 방식) |
| | - 작업 분리 원칙 수립 |
| | - 표준 배포 절차 정의 |
| | - 모니터링 체크리스트 작성 |
| 2026-02-10 | **Git 기반 워크플로우로 전환** |
| | - 파일 전송 방식 → Git pull/push 방식 |
| | - 표준 배포 절차 Git 기반으로 변경 |
| | - CLAUDE.md 파일 추가 |

---

**현재 상태:** ⏸️ 대기 중
**다음 작업:** 개발쪽에서 "Git push 완료" 통보 대기
